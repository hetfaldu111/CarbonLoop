...
@Component({
  ...
  imports: [DatePipe, FormsModule, RouterLink, StatusBadge, LabelPipe, MoneyPipe, TonnesPipe, Alert, CostStackView, Loading, PageHeader],
  template: `
    ...
    @if (a(); as a) {
      ...
      @if (showCancel()) {
        <div class="card mb">
          ...
          <div class="row"><input [(ngModel)]="cancelReason" placeholder="Reason" style="max-width:520px" /><button class="btn btn-danger" (click)="cancel()" [disabled]="!cancelReason.trim() || busy()">Confirm cancellation</button></div>
        </div>
      }
      @if (showShip()) {
        <div class="card mb">
          ...
          <div class="form-row">
            ...
            <div class="field"><label>Volume (t)</label><input type="number" step="0.1" [(ngModel)]="ship.volumeTonnes" /><span class="hint">Remaining under this agreement: {{ remaining() | tonnes }}</span></div>
          ...
          <div class="form-actions"><button class="btn btn-primary" (click)="createShipment()" [disabled]="busy() || ship.volumeTonnes <= 0">Create shipment</button></div>
        </div>
      }
